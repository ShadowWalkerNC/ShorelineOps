...
import { UnitConversionEngine, MASS_TO_GRAMS, VOLUME_TO_ML } from './units'
...
export interface MatchedVendorOffer {
  vendorId: string
  vendorCode: string
  vendorName: string
  vendorSku: string
...
export class PackSizeNormalizer {
   ...
   */
  static normalize(packSize: string, caseCost: number): NormalizedPackSize {
...
export class FuzzyProductMatcher {
  /**
   ...
   */
  static extractTokens(text: string): Set<string> {
  ...
  static scoreMatch(candidateName: string, canonicalName: string): number {
    ...
    if (candTokens.size === 0 || canonTokens.size === 0) return 0
  ...
  ): { canonicalProduct: CanonicalProduct | null; confidence: number } {
    let bestProduct: CanonicalProduct | null = null
...
export class PriceMatrixSolver {
  /**
   * Aggregate matched vendor items into an apples-to-apples price comparison matrix
   */
  static solveMatrix(
    canonicalProducts: CanonicalProduct[],
    matches: MatchedVendorOffer[]
  ): CanonicalPriceMatrixRow[] {
