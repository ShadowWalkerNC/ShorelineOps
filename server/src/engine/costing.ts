...
export type CostLineSource = 'sku' | 'estimated' | 'none'

/** Recipe-level cost provenance. Never silently 'sku-matched' unless every line is. */
export type CostProvenance = 'sku-matched' | 'mixed' | 'estimated' | 'none'

export interface VendorCostEntry {
  vendorSku: string
  unitCost: number      // vendor_items.unit_cost — price for ONE `uom` unit
  uom?: string          // e.g. 'case', 'lb', 'each'
  packSize?: string     // e.g. '6 x 104 oz', '40 lb' — used to derive per-gram cost
  vendorName?: string
}

export interface CostIngredientInput {
  qty: string           // e.g. "2 lbs", "1 cup"
  item: string          // e.g. "chicken breast"
  vendorSku?: string    // matches vendor_items.vendor_sku (client may send vendorItemSku alias)
  vendorItemSku?: string
  estimatedCost?: number // fallback: cost of the listed qty when no SKU match exists
  yieldPct?: number      // AP→EP yield, e.g. 75 (reuse of MRP's AP/EP math)
}

export interface IngredientCostLine {
  item: string
  qty: string
  grams: number            // edible grams at the recipe's base servings
  source: CostLineSource
  vendorSku?: string
  costBasis?: string       // human-readable, e.g. "$24.50/case (DNS-104)"
  lineCost: number          // EP cost of the listed ingredient quantity
}

export interface RecipeCostBreakdown {
  recipeId?: string
  baseServings: number
  batchCost: number        // total EP cost at base servings
...
export interface SlotItemCost {
  itemId: string
  itemName: string
  plateCost: number        // Σ cost_per_serving × portion_multiplier over linked recipes
  recipeCount: number
  provenance: CostProvenance
}

export interface MenuSlotCost {
  slot: string
  items: SlotItemCost[]
  slotPlateCost: number
  provenance: CostProvenance
}

export interface DayCostRollup {
  dayName: string
  weekName: string | null
  slots: MenuSlotCost[]
  perResidentDayCost: number
  residentCount: number
  dailyFoodCost: number
  provenance: CostProvenance
  provenanceDetail: { sku: number; estimated: number; none: number }
  notes: string
}

// Minimal query client — works with the shared pool (pg or sqlite) or a
// transaction client obtained via pool.connect().
export interface CostingDbClient {
...
const WEIGHT_TOKENS: Array<{ re: RegExp; unit: string }> = [
 ...
 */
export function ediblePortionCostPerGram(
  asPurchasedCostPerGram: number,
  yieldPct?: number
): number {
 ...
 */
export function parsePackSizeToGrams(packSize?: string | null): number | null {
  if (!packSize || !packSize.trim()) return null
  ...
  if (multi) {
    ...
    if (gramsPerSub != null && packs > 0 && size > 0) return packs * size * gramsPerSub
  }
  ...
  if (single) {
    ...
    if (gramsPerUnit != null && size > 0) return size * gramsPerUnit
  }

  return null
}

function weightTokenToGrams(token: string): number | null {
  const t = token.trim()
  for (const { re, unit } of WEIGHT_TOKENS) {
    if (re.test(t)) {
      try {
        const { convertedAmount } = UnitConversionEngine.convert(1, unit, 'g', 'vendor pack')
 ...
 */
export function vendorCostPerGram(entry: VendorCostEntry): { perGram: number; basis: string } | null {
...
export function aggregateProvenance(sources: CostLineSource[]): CostProvenance {
  if (sources.length === 0) return 'none'
  const hasSku = sources.includes('sku')
  const hasEst = sources.includes('estimated')
  const hasNone = sources.includes('none')
  if (hasSku && (hasEst || hasNone)) return 'mixed'
  if (hasSku) return 'sku-matched'
  if (hasEst) return 'estimated'
  return 'none'
}

/** Aggregate recipe-level provenances (menu-slot and day level). */
export function aggregateRecipeProvenance(list: CostProvenance[]): CostProvenance {
  ...
  if (hasSku && hasNonSku) return 'mixed'
  if (hasSku) return 'sku-matched'
...
export class RecipeCostingEngine {
   ...
   */
  static buildCatalogIndex(catalog: VendorCostEntry[]): Map<string, VendorCostEntry> {
    const idx = new Map<string, VendorCostEntry>()
  ...
  static costIngredient(
    ing: CostIngredientInput,
    catalog: Map<string, VendorCostEntry>
  ): IngredientCostLine {
   ...
   */
  static ingredientProvenance(
    ingredients: CostIngredientInput[],
    catalogRows: VendorCostEntry[]
  ): CostProvenance {
    const catalog = this.buildCatalogIndex(catalogRows)
    const sources: CostLineSource[] = (ingredients || []).map(ing => {
      const sku = (ing.vendorSku ?? ing.vendorItemSku ?? '').trim().toLowerCase()
      if (sku) {
        const entry = catalog.get(sku)
        if (entry && vendorCostPerGram(entry)) return 'sku'
      }
      const estimated = Number(ing.estimatedCost)
  ...
  static plateCostForItem(
    links: Array<{ costPerServing: number; portionMultiplier: number }>
  ): number {
    const total = links.reduce(
      (s, l) => s + (Number(l.costPerServing) || 0) * (Number(l.portionMultiplier) || 1),
      0
    )
...
export async function fetchVendorCatalog(client: CostingDbClient): Promise<VendorCostEntry[]> {
 ...
 */
export async function recalcRecipeCost(
  client: CostingDbClient,
  recipeId: string
): Promise<RecipeCostBreakdown | null> {
  const { rows } = await client.query(
    `SELECT id, base_servings, ingredients FROM recipes WHERE id = $1`,
    [recipeId]
  )
  if (!rows[0]) return null
  const catalog = await fetchVendorCatalog(client)
  const breakdown = RecipeCostingEngine.costRecipe(
    parseJsonb(rows[0].ingredients) || [],
    parseFloat(rows[0].base_servings) || 1,
    catalog,
    recipeId
  )
...
export async function recalcAllRecipeCosts(client: CostingDbClient): Promise<{
  updated: number
  results: Array<{ id: string; name: string; costPerServing: number; provenance: CostProvenance }>
}> {
  const catalog = await fetchVendorCatalog(client)
 ...
 */
export async function getMenuSlotCosts(
  client: CostingDbClient,
  dayName: string
): Promise<{ weekName: string | null; slots: MenuSlotCost[] }> {
  const { rows: weekRows } = await client.query(
    `SELECT id, name, days FROM menu_weeks WHERE active = TRUE ORDER BY updated_at DESC LIMIT 1`
  )
  if (!weekRows[0]) return { weekName: null, slots: [] }
  const days = parseJsonb(weekRows[0].days) || {}
  const day = days[dayName]
  if (!day) return { weekName: weekRows[0].name, slots: [] }

  const catalog = await fetchVendorCatalog(client)
  const slots: MenuSlotCost[] = []

  for (const slot of MEAL_SLOT_ORDER) {
    const entry = day[slot]
    const itemIds: string[] = (entry && Array.isArray(entry.itemIds)) ? entry.itemIds : []
    if (itemIds.length === 0) continue

    const items: SlotItemCost[] = []
    for (const itemId of itemIds) {
      ...
      const { rows: linkRows } = await client.query(
        `SELECT r.id, r.name, r.cost_per_serving, r.ingredients, r.base_servings,
      ...
      const provenance = aggregateRecipeProvenance(
        linkRows.map(lr =>
          RecipeCostingEngine.ingredientProvenance(parseJsonb(lr.ingredients) || [], catalog)
        )
      )
      items.push({
        itemId,
...
export async function rollupDailyCost(
  ...
  for (const slot of slots) {
    for (const item of slot.items) {
      if (item.provenance === 'sku-matched') provenanceDetail.sku++
      else if (item.provenance === 'mixed') { provenanceDetail.sku++; provenanceDetail.estimated++ }
      else if (item.provenance === 'estimated') provenanceDetail.estimated++
      else provenanceDetail.none++
    }
  }
  ...
  const notes =
    `[auto-rollup] ${dayName} menu from week '${weekName ?? 'none'}': ` +
  ...
  const { rows: existing } = await client.query(
    `SELECT id FROM daily_cost_log WHERE log_date = $1 AND (facility_id IS NULL OR facility_id = $2) LIMIT 1`,
    [logDate, null]
  )
  ...
  if (existing[0]) {
    logId = existing[0].id
    await client.query(
      `UPDATE daily_cost_log
       SET resident_count = $1, food_cost = $2, notes = $3, created_by = COALESCE($4, created_by)
       WHERE id = $5`,
      [residentCount, dailyFoodCost, notes, createdBy ?? null, logId]
    )
  } else {
    const { rows: inserted } = await client.query(
      `INSERT INTO daily_cost_log (log_date, resident_count, food_cost, notes, created_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`,
      [logDate, residentCount, dailyFoodCost, notes, createdBy ?? null]
    )
    logId = inserted[0]?.id
  }
