...
import {
  TRAY_EVENTS,
  MEAL_SLOTS,
  DEFAULT_TRAY_SLA_MINUTES,
  allowedNextEvents,
  computeTrayLines,
  computeMissedTrays,
  lineKey,
  type TrayEventRecord,
} from '../engine/trayTracking'

export const trayrunsRouter = Router()
...
async function audit(
  ...
  await pool.query(
    `INSERT INTO audit_log (action, user_id, resource_id, resource_type, outcome, details)
     VALUES ($1, $2, $3, 'tray_tracking', $4, $5)`,
    [action, req.userId ?? null, resourceId, outcome, details ? JSON.stringify(details) : null]
  )
}
...
function toEvent(row: any): TrayEventRecord {
  return {
    id:          row.id,
    run_id:      row.run_id,
...
async function fetchRun(id: string): Promise<any | null> {
  const { rows } = await pool.query('SELECT * FROM tray_runs WHERE id = $1', [id])
  return rows[0] ?? null
}

async function fetchEvents(runId: string): Promise<TrayEventRecord[]> {
  const { rows } = await pool.query(
    'SELECT * FROM tray_events WHERE run_id = $1 ORDER BY at ASC, created_at ASC',
    [runId]
  )
...
async function resolveResidentByTicket(ticketId: string): Promise<any | null> {
  if (!ticketId || !ticketId.startsWith('TKT-')) return null
  const prefix = ticketId.split('-')[1]
  if (!prefix) return null
  // CAST(...) instead of ::text — the SQLite translation layer cannot parse :: casts.
  const { rows } = await pool.query(
    `SELECT id, name, room, is_npo, npo_reason FROM residents WHERE CAST(id AS TEXT) LIKE $1 LIMIT 1`,
    [`${prefix}%`]
  )
  return rows[0] ?? null
}

async function residentMap(ids: (string | null)[]): Promise<Map<string, any>> {
  const uniq = [...new Set(ids.filter((i): i is string => !!i))]
  const map = new Map<string, any>()
  if (uniq.length === 0) return map
  const placeholders = uniq.map((_, i) => `$${i + 1}`).join(',')
  const { rows } = await pool.query(
    `SELECT id, name, room, diet_type, is_npo FROM residents WHERE id IN (${placeholders})`,
    uniq
  )
...
function parseSla(req: AuthRequest): number {
  const raw = typeof req.query.slaMinutes === 'string' ? Number(req.query.slaMinutes) : DEFAULT_TRAY_SLA_MINUTES
  if (!Number.isFinite(raw) || raw < 1 || raw > 480) return DEFAULT_TRAY_SLA_MINUTES
...
trayrunsRouter.get('/', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (mealSlot) {
      if (!(MEAL_SLOTS as readonly string[]).includes(mealSlot)) {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM tray_runs ${where} ORDER BY service_date DESC, created_at DESC LIMIT 100`,
      params
    )
...
trayrunsRouter.post('/ensure', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    const data = EnsureBodySchema.parse(req.body)
    const serviceDate = data.serviceDate ?? todayString()
    const wing = data.wing ?? ''
    const { rows } = await pool.query(
      `SELECT * FROM tray_runs WHERE service_date = $1 AND meal_slot = $2 AND wing = $3`,
      [serviceDate, data.mealSlot, wing]
    )
    if (rows[0]) return res.json({ run: toRun(rows[0]), created: false })

    const id = randomUUID()
    await pool.query(
      `INSERT INTO tray_runs (id, meal_slot, service_date, wing, created_by)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, data.mealSlot, serviceDate, wing, req.userId ?? null]
    )
...
trayrunsRouter.post('/', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    const data = RunBodySchema.parse(req.body)
    const id = randomUUID()
    try {
      await pool.query(
        `INSERT INTO tray_runs (id, meal_slot, service_date, wing, notes, created_by)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [id, data.mealSlot, data.serviceDate, data.wing, data.notes, req.userId ?? null]
      )
    } catch (err: any) {
    ...
    const created = await fetchRun(id)
    if (!created) return res.status(500).json({ error: 'Failed to read created run' })
...
trayrunsRouter.get('/missed', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    ...
    const { rows: runs } = await pool.query(
      'SELECT * FROM tray_runs WHERE service_date = $1 ORDER BY created_at ASC',
      [serviceDate]
    )
    const missed: any[] = []
    for (const run of runs) {
      const events = await fetchEvents(run.id)
      const lines = computeTrayLines(events)
      const hits = computeMissedTrays(lines, slaMinutes)
...
// GET /api/trayruns/:id
trayrunsRouter.get('/:id', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    const run = await fetchRun(req.params.id)
    if (!run) return res.status(404).json({ error: 'Tray run not found' })
    res.json(toRun(run))
  } catch (err) { next(err) }
})

// GET /api/trayruns/:id/checklist — residents on the run with latest event per tray line
trayrunsRouter.get('/:id/checklist', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    const run = await fetchRun(req.params.id)
    if (!run) return res.status(404).json({ error: 'Tray run not found' })
    const slaMinutes = parseSla(req)
    ...
    const missed = computeMissedTrays(lines, slaMinutes)
...
trayrunsRouter.post('/:id/events', requireRole('staff'), async (req: AuthRequest, res, next) => {
  ...
  try {
    ...
    const verification = await verifyTray(data.rawQrPayload, client, true)
    if (verification.status !== 'VALID' || !verification.claims) return res.status(409).json({ error: verification.message, status: verification.status })
    const claims = verification.claims
    if ((data.residentId && data.residentId !== claims.residentId) || (data.ticketId && data.ticketId !== claims.ticketId) || claims.mealSlot !== run.meal_slot || claims.serviceDate !== (run.service_date instanceof Date ? run.service_date.toISOString().slice(0, 10) : String(run.service_date).slice(0, 10))) return res.status(409).json({ error: 'Ticket does not match this resident, meal, or service date' })
    ...
    const existing: TrayEventRecord[] = (await client.query('SELECT * FROM tray_events WHERE run_id = $1 ORDER BY at ASC, created_at ASC', [run.id])).rows.map(toEvent)
    const newKey = lineKey({ resident_id: resident?.id ?? null, ticket_id: ticketId })
    const lineHist = existing
      .filter((e) => lineKey(e) === newKey)
      .map((e) => e.event)
    const allowed = allowedNextEvents(lineHist)
    ...
    await client.query(
      `INSERT INTO tray_events (id, run_id, resident_id, ticket_id, event, by, note)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [id, run.id, resident?.id ?? null, ticketId, data.event, req.userId ?? null, note]
    )
    ...
    if (!rows[0]) return res.status(500).json({ error: 'Failed to read created event' })
