    ...
    try {
      ...
      if (action === 'APPROVED_BY_RD' && triageItem.resident_id) {
        ...
        if (triageItem.change_type === 'DIET_ORDER') {
          ...
          await client.query(
            'UPDATE residents SET diet_type = $1, profile_version = profile_version + 1, updated_at = NOW() WHERE id = $2',
          ...
          await client.query(
            'UPDATE residents SET texture = $1, profile_version = profile_version + 1, updated_at = NOW() WHERE id = $2',
            [payload.texture, triageItem.resident_id]
          )
        } else if (triageItem.change_type === 'NPO_ORDER') {
          await client.query(
            "UPDATE residents SET is_npo = true, npo_reason = 'EHR Physician Order', profile_version = profile_version + 1, updated_at = NOW() WHERE id = $1",
          ...
          const { rows: [resident] } = await client.query(
            'SELECT allergies FROM residents WHERE id = $1',
            [triageItem.resident_id]
          )
          if (!resident) {
            await client.query('ROLLBACK')
          ...
          await client.query(
            'UPDATE residents SET allergies = $1, profile_version = profile_version + 1, updated_at = NOW() WHERE id = $2',
            [current, triageItem.resident_id]
          )
        } else {
          await client.query('ROLLBACK')
      ...
      await client.query(
        'UPDATE ehr_reconciliation_queue SET status = $1, resolved_by = $2, resolved_at = NOW() WHERE id = $3',
      ...
      try { if (started) await client.query('ROLLBACK') } catch { /* already closed */ }
      throw txErr
    } finally {
      client.release()
    }
