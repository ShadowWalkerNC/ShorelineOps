...
const HACCP_EQUIPMENT_TYPES = ['fridge', 'freezer', 'dishwasher', 'hot-hold'] as const
const HACCP_CHECK_FREQUENCIES = ['shift', 'daily', 'weekly'] as const
...
const FREQUENCY_HOURS: Record<string, number> = { shift: 8, daily: 24, weekly: 168 }

interface HaccpEquipmentRow {
  id: string
  name: string
  type: string
...
async function auditHaccp(
  ...
  await pool.query(
    ...
    [action, userId, resourceId, resourceType, JSON.stringify(details)]
  )
}

async function getEquipment(id: string): Promise<HaccpEquipmentRow | null> {
  const { rows } = await pool.query(`SELECT * FROM haccp_equipment WHERE id = $1`, [id])
  return rows[0] ?? null
}

/** Due/overdue computation per equipment against its check frequency. */
async function buildSchedule(): Promise<Array<HaccpEquipmentRow & {
  ...
  const { rows: equipment } = await pool.query(
    `SELECT * FROM haccp_equipment WHERE active = $1 ORDER BY name`, [true]
  )
  const now = Date.now()
  const out: Array<HaccpEquipmentRow & {
    last_log_id: string | null
    last_recorded_at: string | null
    last_temp_f: number | null
    next_due_at: string | null
  ...
  for (const eq of equipment) {
    const { rows: last } = await pool.query(
      `SELECT id, recorded_at, temp_f FROM haccp_logs
    ...
    if (lastAt == null || Number.isNaN(lastAt)) {
      ...
      else if (now >= dueAt - 30 * 60 * 1000) status = 'due'
      else status = 'ok'
    }
    out.push({
      ...eq,
...
hardwareRouter.get('/haccp/equipment', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM haccp_equipment ${includeInactive ? '' : 'WHERE active = $1'} ORDER BY name`,
...
hardwareRouter.post('/haccp/equipment', requireRole('manager'), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (!(HACCP_EQUIPMENT_TYPES as readonly string[]).includes(type)) {
    ...
    if (!(HACCP_CHECK_FREQUENCIES as readonly string[]).includes(checkFrequency)) {
      return res.status(400).json({ error: `checkFrequency must be one of ${HACCP_CHECK_FREQUENCIES.join(', ')}` })
    }
    const id = randomUUID()
    await pool.query(
      `INSERT INTO haccp_equipment (id, name, type, target_temp_f, check_frequency)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, name.trim(), type, target, checkFrequency]
    )
    const saved = await getEquipment(id)
    await auditHaccp(req.userId ?? null, 'HACCP_EQUIPMENT_CREATE', id, 'haccp_equipment',
      { name: name.trim(), type, targetTempF: target, checkFrequency })
...
hardwareRouter.patch('/haccp/equipment/:id', requireRole('manager'), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (checkFrequency !== undefined && !(HACCP_CHECK_FREQUENCIES as readonly string[]).includes(checkFrequency)) {
      return res.status(400).json({ error: `checkFrequency must be one of ${HACCP_CHECK_FREQUENCIES.join(', ')}` })
    }
    ...
    await auditHaccp(req.userId ?? null, 'HACCP_EQUIPMENT_UPDATE', req.params.id, 'haccp_equipment',
      { name: name ?? undefined, targetTempF: target ?? undefined, checkFrequency: checkFrequency ?? undefined, active: active ?? undefined })
    res.json(saved)
  } catch (err) { next(err) }
})
...
hardwareRouter.get('/haccp/logs', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const params: any[] = []
    const push = (cond: string, val: any) => { params.push(val); conditions.push(cond.replace('?', `$${params.length}`)) }

    if (start) push(`date(l.recorded_at) >= date(?)`, start)
    if (end) push(`date(l.recorded_at) <= date(?)`, end)
    if (equipmentId) push(`l.equipment_id = ?`, equipmentId)
    if (violationsOnly === 'true') push(`l.compliant = ?`, false)
    if (checkType && ['food', 'equipment'].includes(checkType as string)) push(`l.check_type = ?`, checkType)
    ...
    const { rows } = await pool.query(
      `SELECT l.*, e.name AS equipment_name, u.name AS recorded_by_name
       FROM haccp_logs l
...
hardwareRouter.post('/haccp/log-temp', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    const {
      ...
      correctiveAction = '', source = 'manual', probeDevice, recordedBy,
    } = req.body ?? {}
    ...
    let equipment: HaccpEquipmentRow | null = null
    let target: number
    if (checkType === 'equipment') {
      ...
      if (equipmentId) {
        equipment = await getEquipment(equipmentId)
        if (equipment && equipment.active) target = Number(equipment.target_temp_f)
      }
    }
    if (!Number.isFinite(target)) {
      return res.status(400).json({ error: 'targetTempF must be a number (°F)' })
    ...
    await pool.query(
      ...
      [id, checkType, itemName.trim(), equipment?.id ?? null, measured, target,
       compliant, violationType, action,
       source, source === 'probe' ? probeDevice.trim() : null,
       by]
    )
    const { rows: savedRows } = await pool.query(
      `SELECT l.*, e.name AS equipment_name FROM haccp_logs l
       LEFT JOIN haccp_equipment e ON e.id = l.equipment_id
       WHERE l.id = $1`, [id]
    )
    ...
    await auditHaccp(req.userId ?? null, 'HACCP_LOG_TEMP', id, 'haccp_log',
      { checkType, itemName: itemName.trim(), equipmentId: equipment?.id ?? null, tempF: measured, targetTempF: target, compliant, violationType })
    ...
    if (!compliant) {
      const event: HaccpTempViolationEvent = {
        ...
        payload: {
          probeId: source === 'probe' ? (probeDevice as string) : 'manual-entry',
          ...
          violationType: violationType as HaccpTempViolationEvent['payload']['violationType'],
...
hardwareRouter.patch('/haccp/logs/:id', requireRole('staff'), async (req: AuthRequest, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`SELECT * FROM haccp_logs WHERE id = $1`, [req.params.id])
    if (!rows[0]) return res.status(404).json({ error: 'log not found' })
    await pool.query(
      `UPDATE haccp_logs SET corrective_action = $1 WHERE id = $2`,
      [correctiveAction.trim(), req.params.id]
    )
    const { rows: saved } = await pool.query(`SELECT * FROM haccp_logs WHERE id = $1`, [req.params.id])
    await auditHaccp(req.userId ?? null, 'HACCP_CORRECTIVE_ACTION', req.params.id, 'haccp_log',
      { correctiveAction: correctiveAction.trim() })
    res.json(saved[0])
  } catch (err) { next(err) }
})

function deriveViolationType(
  checkType: string,
  equipment: HaccpEquipmentRow | null,
  target: number
): 'hot_hold_below_140' | 'cold_hold_above_41' | 'cook_below_165' {
  if (equipment && COLD_EQUIPMENT.has(equipment.type)) return 'cold_hold_above_41'
  if (checkType === 'food' && target >= 165) return 'cook_below_165'
