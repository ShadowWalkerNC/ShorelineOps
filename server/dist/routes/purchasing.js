...
// ─── Order lines ──────────────────────────────────────────────────────────
/** POST /api/purchasing/orders/:id/lines */
exports.purchasingRouter.post('/orders/:id/lines', async (req, res, next) => {
    const { id } = req.params;
    ...
    try {
        const { rows } = await pool_1.pool.query(`INSERT INTO purchase_order_lines (purchase_order_id, vendor_item_id, qty_ordered, unit_cost, notes)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [id, vendorItemId, qtyOrdered, unitCost ?? null, notes]);
        res.status(201).json(rows[0]);
    }
    catch (e) {
        next(e);
...
/** PUT /api/purchasing/orders/:id/lines/:lineId */
exports.purchasingRouter.put('/orders/:id/lines/:lineId', async (req, res, next) => {
    const { lineId } = req.params;
    const { qtyOrdered, qtyReceived, unitCost, notes } = req.body;
    try {
        const { rows } = await pool_1.pool.query(`UPDATE purchase_order_lines SET
         qty_ordered = COALESCE($1, qty_ordered),
         qty_received = COALESCE($2, qty_received),
         unit_cost = COALESCE($3, unit_cost),
         notes = COALESCE($4, notes)
       WHERE id = $5 RETURNING *`, [qtyOrdered, qtyReceived, unitCost, notes, lineId]);
        if (!rows.length)
...
/** DELETE /api/purchasing/orders/:id/lines/:lineId */
exports.purchasingRouter.delete('/orders/:id/lines/:lineId', async (req, res, next) => {
    const { lineId } = req.params;
    try {
        await pool_1.pool.query(`DELETE FROM purchase_order_lines WHERE id = $1`, [lineId]);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
