...
exports.purchasingRouter = void 0;
/**
...
exports.purchasingRouter = (0, express_1.Router)();
// ─── Helper ──────────────────────────────────────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════════════════
/** GET /api/purchasing/vendors */
exports.purchasingRouter.get('/vendors', async (_req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`SELECT * FROM vendors ORDER BY name ASC`);
        res.json(rows);
    }
    catch (e) {
        next(e);
...
/** POST /api/purchasing/vendors */
exports.purchasingRouter.post('/vendors', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { name, code, phone = '', email = '', website = '', notes = '' } = req.body;
    if (!name || !code)
        return err(res, 400, 'name and code are required');
    try {
        const { rows } = await pool_1.pool.query(`INSERT INTO vendors (name, code, phone, email, website, notes)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [name, code.toLowerCase().replace(/\s+/g, '-'), phone, email, website, notes]);
...
/** PUT /api/purchasing/vendors/:id */
exports.purchasingRouter.put('/vendors/:id', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id } = req.params;
    const { name, phone, email, website, notes, active } = req.body;
    try {
        const { rows } = await pool_1.pool.query(`UPDATE vendors SET
         ...
         updated_at = NOW()
       WHERE id = $7 RETURNING *`, [name, phone, email, website, notes, active, id]);
        if (!rows.length)
            return err(res, 404, 'Vendor not found');
        res.json(rows[0]);
    }
    catch (e) {
        next(e);
...
// ═══════════════════════════════════════════════════════════════════════════
/** GET /api/purchasing/items?vendorId= */
exports.purchasingRouter.get('/items', async (req, res, next) => {
    const { vendorId } = req.query;
    try {
        const { rows } = await pool_1.pool.query(`SELECT vi.*, v.name AS vendor_name, v.code AS vendor_code
       FROM vendor_items vi
       JOIN vendors v ON v.id = vi.vendor_id
       WHERE ($1::uuid IS NULL OR vi.vendor_id = $1)
         AND vi.active = true
       ORDER BY vi.category, vi.name`, [vendorId || null]);
        res.json(rows);
    }
    catch (e) {
        next(e);
...
/** POST /api/purchasing/items */
exports.purchasingRouter.post('/items', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { vendorId, vendorSku, name, brand = '', packSize = '', uom = 'case', category = '', unitCost = 0 } = req.body;
    if (!vendorId || !vendorSku || !name)
        return err(res, 400, 'vendorId, vendorSku, and name are required');
    try {
        const { rows } = await pool_1.pool.query(`INSERT INTO vendor_items (vendor_id, vendor_sku, name, brand, pack_size, uom, category, unit_cost)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`, [vendorId, vendorSku, name, brand, packSize, uom, category, unitCost]);
        res.status(201).json(rows[0]);
    }
    catch (e) {
        next(e);
...
/** PUT /api/purchasing/items/:id */
exports.purchasingRouter.put('/items/:id', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id } = req.params;
    const { name, brand, packSize, uom, category, unitCost, active } = req.body;
    try {
        const { rows } = await pool_1.pool.query(`UPDATE vendor_items SET
         name = COALESCE($1, name),
         ...
         updated_at = NOW()
       WHERE id = $8 RETURNING *`, [name, brand, packSize, uom, category, unitCost, active, id]);
        if (!rows.length)
            return err(res, 404, 'Item not found');
        res.json(rows[0]);
    }
    catch (e) {
        next(e);
...
exports.purchasingRouter.delete('/items/:id', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id } = req.params;
    try {
        await pool_1.pool.query(`UPDATE vendor_items SET active = false, updated_at = NOW() WHERE id = $1`, [id]);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
// ═══════════════════════════════════════════════════════════════════════════
/** GET /api/purchasing/order-guide?vendorId= */
exports.purchasingRouter.get('/order-guide', async (req, res, next) => {
    const { vendorId } = req.query;
    try {
        const { rows } = await pool_1.pool.query(`SELECT og.*,
       ...
       ORDER BY og.sort_group, vi.category, vi.name`, [vendorId || null]);
        res.json(rows);
    }
    catch (e) {
        next(e);
...
exports.purchasingRouter.post('/order-guide', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { vendorId, vendorItemId, parLevel = 0, onHand = 0, avgUsage, sortGroup = '' } = req.body;
    if (!vendorId || !vendorItemId)
        return err(res, 400, 'vendorId and vendorItemId required');
    try {
        const { rows } = await pool_1.pool.query(`INSERT INTO order_guides (vendor_id, vendor_item_id, par_level, on_hand, avg_usage, sort_group)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (facility_id, vendor_item_id) DO UPDATE
         SET par_level = EXCLUDED.par_level,
             ...
             updated_at = NOW()
       RETURNING *`, [vendorId, vendorItemId, parLevel, onHand, avgUsage ?? null, sortGroup]);
        res.status(201).json(rows[0]);
    }
    catch (e) {
        next(e);
...
exports.purchasingRouter.put('/order-guide/:id', async (req, res, next) => {
    const { id } = req.params;
    const { parLevel, onHand, avgUsage, sortGroup } = req.body;
    try {
        const { rows } = await pool_1.pool.query(`UPDATE order_guides SET
         par_level = COALESCE($1, par_level),
         on_hand = COALESCE($2, on_hand),
         avg_usage = COALESCE($3, avg_usage),
         sort_group = COALESCE($4, sort_group),
         updated_at = NOW()
       WHERE id = $5 RETURNING *`, [parLevel, onHand, avgUsage, sortGroup, id]);
        if (!rows.length)
            return err(res, 404, 'Order guide entry not found');
        res.json(rows[0]);
    }
    catch (e) {
        next(e);
...
/** DELETE /api/purchasing/order-guide/:id */
exports.purchasingRouter.delete('/order-guide/:id', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id } = req.params;
    try {
        await pool_1.pool.query(`DELETE FROM order_guides WHERE id = $1`, [id]);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
 ...
 */
exports.purchasingRouter.post('/import-guide', async (req, res, next) => {
    const { vendorId, items = [] } = req.body;
    if (!vendorId || !Array.isArray(items))
        return err(res, 400, 'vendorId and items array required');
    const client = await pool_1.pool.connect();
    try {
        await client.query('BEGIN');
        let importedCount = 0;
        for (const item of items) {
            if (!item.vendorSku || !item.name)
                continue;
            // Upsert vendor item into catalog
            const { rows: [vItem] } = await client.query(`INSERT INTO vendor_items (vendor_id, vendor_sku, name, brand, pack_size, uom, category, unit_cost)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         ...
         RETURNING id`, [
            ...
            if (vItem) {
                await client.query(`INSERT INTO order_guides (vendor_id, vendor_item_id, par_level, on_hand, sort_group)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (facility_id, vendor_item_id) DO UPDATE
             SET par_level = EXCLUDED.par_level,
                 on_hand = EXCLUDED.on_hand,
...
// SUGGESTED ORDER
// ═══════════════════════════════════════════════════════════════════════════
const dietaryDemand_1 = require("../integrations/dietaryDemand");
const demandEngine = new dietaryDemand_1.DietaryDemandEngine();
/**
 ...
 */
exports.purchasingRouter.post('/suggested-order', async (req, res, next) => {
    const { vendorId } = req.body;
    if (!vendorId)
        return err(res, 400, 'vendorId required');
    try {
        const { rows } = await pool_1.pool.query(`SELECT og.*,
              vi.name AS item_name, vi.vendor_sku, vi.pack_size, vi.uom, vi.unit_cost, vi.category,
       ...
       ORDER BY vi.category, vi.name`, [vendorId]);
 ...
 */
exports.purchasingRouter.post('/clinical-suggested-order', async (req, res, next) => {
    const { vendorId } = req.body;
    if (!vendorId)
        return err(res, 400, 'vendorId required');
    try {
        // 1. Fetch live active resident profiles
        const { rows: residentRows } = await pool_1.pool.query(`SELECT status, diet_type AS "dietType", texture, allergies, beverages FROM residents WHERE status = 'Active'`);
        const census = demandEngine.calculateCensusMetrics(residentRows);
        // 2. Fetch vendor items and order guide pars
        const { rows: guideRows } = await pool_1.pool.query(`SELECT og.*,
              vi.name AS item_name, vi.vendor_sku, vi.pack_size, vi.uom, vi.unit_cost, vi.category,
              v.name  AS vendor_name
       FROM order_guides og
       JOIN vendor_items vi ON vi.id = og.vendor_item_id
 ...
 */
exports.purchasingRouter.post('/mrp-order', async (req, res, next) => {
    try {
        const { vendorId } = req.body;
        ...
        const exploded = mrp_1.MrpDemandForecastEngine.explodeBillOfMaterials(forecast.meals);
        // 4. Fetch vendor inventory and order guide stock
        const { rows: guideRows } = await pool_1.pool.query(`
      ...
      WHERE ($1::uuid IS NULL OR og.vendor_id = $1)
    `, [vendorId || null]);
        const stockItems = guideRows.map(g => ({
        ...
        const recommendations = mrp_1.MrpDemandForecastEngine.calculateMaterialRequirements(exploded, stockItems);
        res.json({
 ...
 */
exports.purchasingRouter.post('/rollup-usage', (0, requireAuth_1.requireRole)('staff'), async (_req, res, next) => {
    try {
        const result = await (0, nightlyForecast_1.rollupAvgUsage)();
        res.json(result);
    }
    catch (e) {
        next(e);
...
/** GET /api/purchasing/orders */
exports.purchasingRouter.get('/orders', async (_req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`SELECT po.*, v.name AS vendor_name, v.code AS vendor_code,
...
exports.purchasingRouter.post('/orders', (0, requireAuth_1.requireRole)('staff'), async (req, res, next) => {
    const ar = req;
    ...
    try {
        await client.query('BEGIN');
        const { rows: [order] } = await client.query(`INSERT INTO purchase_orders (vendor_id, order_date, expected_date, notes, created_by, status)
       VALUES ($1, $2, $3, $4, $5, 'draft')
       RETURNING *`, [vendorId, orderDate ?? new Date().toISOString().slice(0, 10), expectedDate ?? null, notes, ar.userId ?? null]);
        for (const line of lines) {
            await client.query(`INSERT INTO purchase_order_lines (purchase_order_id, vendor_item_id, qty_ordered, unit_cost, notes)
         VALUES ($1, $2, $3, $4, $5)`, [order.id, line.vendorItemId, line.qtyOrdered, line.unitCost ?? null, line.notes ?? '']);
        }
        await client.query(`INSERT INTO audit_log (action, user_id, resource_id, resource_type, outcome, details)
       VALUES ('PO_CREATE', $1, $2, 'purchase_order', 'success', $3)`, [ar.userId ?? null, order.id, JSON.stringify({ vendorId, lineCount: lines.length })]);
        await client.query('COMMIT');
...
/** GET /api/purchasing/orders/:id */
exports.purchasingRouter.get('/orders/:id', async (req, res, next) => {
    const { id } = req.params;
    try {
        const { rows: [order] } = await pool_1.pool.query(`SELECT po.*, v.name AS vendor_name, v.code AS vendor_code
       FROM purchase_orders po
       JOIN vendors v ON v.id = po.vendor_id
       WHERE po.id = $1`, [id]);
        if (!order)
            return err(res, 404, 'Order not found');
        const { rows: lines } = await pool_1.pool.query(`SELECT pol.*, vi.name AS item_name, vi.vendor_sku, vi.pack_size, vi.uom,
       ...
       ORDER BY vi.category, vi.name`, [id]);
...
/** PUT /api/purchasing/orders/:id */
exports.purchasingRouter.put('/orders/:id', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id } = req.params;
    const { status, expectedDate, notes } = req.body;
    if (status !== undefined)
        return err(res, 400, 'Use the dedicated approval, submission, or cancellation workflow');
    try {
        const { rows } = await pool_1.pool.query(`UPDATE purchase_orders SET
         ...
         updated_at = NOW()
       WHERE id = $4 AND status = 'draft' RETURNING *`, [status, expectedDate, notes, id]);
        if (!rows.length)
            return err(res, 404, 'Order not found');
        res.json(rows[0]);
    }
...
/** DELETE /api/purchasing/orders/:id */
exports.purchasingRouter.delete('/orders/:id', (0, requireAuth_1.requireRole)('admin'), async (req, res, next) => {
    const { id } = req.params;
    try {
        await pool_1.pool.query(`UPDATE purchase_orders SET status = 'cancelled', updated_at = NOW() WHERE id = $1`, [id]);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
async function findOrder(client, id) {
    const { rows: [order] } = await client.query(`SELECT * FROM purchase_orders WHERE id = $1${pool_1.databaseDialect === 'postgres' ? ' FOR UPDATE' : ''}`, [id]);
    return order ?? null;
}
/**
 ...
 */
exports.purchasingRouter.post('/orders/:id/approve', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const ar = req;
    const { id } = req.params;
    const client = await pool_1.pool.connect();
    try {
        ...
        await client.query(`UPDATE purchase_orders SET status = 'approved', updated_at = NOW()
       WHERE id = $1 AND status = 'draft'`, [id]);
        ...
        await client.query(`INSERT INTO audit_log (action, user_id, resource_id, resource_type, outcome, details)
       VALUES ('PO_APPROVE', $1, $2, 'purchase_order', 'success', $3)`, [ar.userId ?? null, id, JSON.stringify({ from: 'draft', to: 'approved', vendorId: order.vendor_id })]);
        await client.query('COMMIT');
        res.json(moved);
    }
    catch (e) {
 ...
 */
exports.purchasingRouter.post('/orders/:id/submit', (0, requireAuth_1.requireRole)('staff'), async (req, res, next) => {
    const ar = req;
    const { id } = req.params;
    const client = await pool_1.pool.connect();
    try {
        ...
        await client.query(`UPDATE purchase_orders SET status = 'submitted', updated_at = NOW()
       WHERE id = $1 AND status = 'approved'`, [id]);
        ...
        await client.query(`INSERT INTO audit_log (action, user_id, resource_id, resource_type, outcome, details)
       VALUES ('PO_SUBMIT', $1, $2, 'purchase_order', 'success', $3)`, [ar.userId ?? null, id, JSON.stringify({ from: 'approved', to: 'submitted', vendorId: order.vendor_id })]);
        await client.query('COMMIT');
        res.json(moved);
    }
    catch (e) {
...
// ─── Order lines ──────────────────────────────────────────────────────────
/** POST /api/purchasing/orders/:id/lines */
exports.purchasingRouter.post('/orders/:id/lines', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id } = req.params;
    const { vendorItemId, qtyOrdered, unitCost, notes = '' } = req.body;
    if (typeof qtyOrdered !== 'number' || !Number.isFinite(qtyOrdered) || qtyOrdered <= 0 || (unitCost != null && (typeof unitCost !== 'number' || !Number.isFinite(unitCost) || unitCost < 0)))
        return err(res, 400, 'Invalid quantity or cost');
    if (!vendorItemId || qtyOrdered == null)
        return err(res, 400, 'vendorItemId and qtyOrdered required');
    const client = await pool_1.pool.connect();
    try {
        await client.query('BEGIN');
        // F5: lines change only while the parent order is a mutable draft.
        const { rows: [order] } = await client.query(`SELECT id, status FROM purchase_orders WHERE id = $1${pool_1.databaseDialect === 'postgres' ? ' FOR UPDATE' : ''}`, [id]);
        if (!order) {
            await client.query('ROLLBACK');
        ...
        if (order.status !== 'draft') {
            await client.query('ROLLBACK');
            return err(res, 409, 'Order lines change only while the order is a draft — changes require reapproval');
        }
        const { rows } = await client.query('INSERT INTO purchase_order_lines (purchase_order_id, vendor_item_id, qty_ordered, unit_cost, notes) VALUES ($1, $2, $3, $4, $5) RETURNING *', [id, vendorItemId, qtyOrdered, unitCost ?? null, notes]);
        await client.query('COMMIT');
        res.status(201).json(rows[0]);
    }
    catch (e) {
...
/** PUT /api/purchasing/orders/:id/lines/:lineId */
exports.purchasingRouter.put('/orders/:id/lines/:lineId', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    ...
    try {
        ...
        const { rows } = await client.query('UPDATE purchase_order_lines SET qty_ordered = COALESCE($1, qty_ordered), unit_cost = COALESCE($2, unit_cost), notes = COALESCE($3, notes) WHERE id = $4 AND purchase_order_id = $5 RETURNING *', [qtyOrdered, unitCost, notes, lineId, id]);
        if (!rows.length) {
            await client.query('ROLLBACK');
            return err(res, 404, 'Line not found');
        }
        await client.query('COMMIT');
        res.json(rows[0]);
    }
...
/** DELETE /api/purchasing/orders/:id/lines/:lineId */
exports.purchasingRouter.delete('/orders/:id/lines/:lineId', (0, requireAuth_1.requireRole)('manager'), async (req, res, next) => {
    const { id, lineId } = req.params;
    const client = await pool_1.pool.connect();
    try {
        await client.query('BEGIN');
 ...
 */
function toGuideUnits(qty, fromUnit, guideUnit, itemName) {
    const from = units_1.UnitConversionEngine.normalizeUnit(fromUnit || '');
    const to = units_1.UnitConversionEngine.normalizeUnit(guideUnit || '');
    const isMass = (u) => u in units_1.MASS_TO_GRAMS;
    const isVol = (u) => u in units_1.VOLUME_TO_ML;
    const engineHandles = from === to ||
        ((isMass(from) || isVol(from)) && (isMass(to) || isVol(to))) ||
        (from === '#10 can' && isMass(to)) ||
        (to === '#10 can' && isMass(from));
    if (engineHandles) {
        const { convertedAmount } = units_1.UnitConversionEngine.convert(qty, from, to, itemName);
 ...
 */
exports.purchasingRouter.post('/orders/:id/lines/:lineId/receive', (0, requireAuth_1.requireRole)('staff'), async (req, res, next) => {
    const ar = req;
    const { id, lineId } = req.params;
    const { qtyReceived, qtyUnit, note = '' } = req.body ?? {};
    const total = Number(qtyReceived);
    if (!Number.isFinite(total) || total < 0)
        return err(res, 400, 'qtyReceived must be a non-negative number');
    const client = await pool_1.pool.connect();
    try {
        await client.query('BEGIN');
        const { rows: [line] } = await client.query(`SELECT pol.*, vi.vendor_sku, vi.name AS item_name, vi.uom AS line_uom
       FROM purchase_order_lines pol
       JOIN vendor_items vi ON vi.id = pol.vendor_item_id
       WHERE pol.id = $1 AND pol.purchase_order_id = $2`, [lineId, id]);
        if (!line) {
            await client.query('ROLLBACK');
            return err(res, 404, 'PO line not found');
        }
        const { rows: [guide] } = await client.query(`SELECT og.*, vi.uom AS guide_uom
       FROM order_guides og
       JOIN vendor_items vi ON vi.id = og.vendor_item_id
       WHERE og.vendor_item_id = $1
       LIMIT 1`, [line.vendor_item_id]);
        if (!guide) {
            await client.query('ROLLBACK');
        ...
        const baseUnit = units_1.UnitConversionEngine.normalizeUnit(line.line_uom || 'each');
        ...
        await client.query(`UPDATE purchase_order_lines SET qty_received = $1 WHERE id = $2`, [totalBase, lineId]);
        await client.query(`UPDATE order_guides SET on_hand = on_hand + $1, updated_at = NOW() WHERE id = $2`, [conv.qty, guide.id]);
        ...
        const { rows: [recheck] } = await client.query(`SELECT on_hand FROM order_guides WHERE id = $1`, [guide.id]);
        ...
        await client.query(`INSERT INTO inventory_transactions (id, item_id, type, qty, unit, user_id, note)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
        ...
        const { rows: pending } = await client.query(`SELECT 1 FROM purchase_order_lines
       WHERE purchase_order_id = $1 AND (qty_received IS NULL OR qty_received < qty_ordered)
       LIMIT 1`, [id]);
        let orderStatus;
        if (!pending.length) {
            const { rows: [o] } = await client.query(`UPDATE purchase_orders SET status = 'received', updated_at = NOW()
         WHERE id = $1 AND status <> 'cancelled' RETURNING status`, [id]);
 ...
 */
exports.purchasingRouter.get('/orders/:id/export-csv', async (req, res, next) => {
    const { id } = req.params;
    try {
        const { rows: [order] } = await pool_1.pool.query(`SELECT po.*, v.name AS vendor_name FROM purchase_orders po JOIN vendors v ON v.id = po.vendor_id WHERE po.id = $1`, [id]);
        if (!order)
            return err(res, 404, 'Order not found');
        const { rows: lines } = await pool_1.pool.query(`SELECT pol.qty_ordered, vi.name AS item_name, vi.vendor_sku, vi.pack_size, vi.uom, v.name AS vendor_name
       FROM purchase_order_lines pol
       JOIN vendor_items vi ON vi.id = pol.vendor_item_id
       ...
       ORDER BY vi.category, vi.name`, [id]);
        ...
        const csvRows = lines.map(l => `"${order.vendor_name}","${l.item_name}","${l.vendor_sku}","${l.pack_size}","${l.uom}",${l.qty_ordered}`).join('\n');
 ...
 */
exports.purchasingRouter.get('/invoices', (0, requireTier_1.requireTier)('enterprise'), async (_req, res, next) => {
    try {
 ...
 */
exports.purchasingRouter.post('/invoices/evaluate', (0, requireTier_1.requireTier)('enterprise'), async (req, res, next) => {
    try {
        ...
        const report = invoicing_1.ThreeWayInvoiceMatchingEngine.evaluateThreeWayMatch({
            invoiceNumber,
            vendorName,
