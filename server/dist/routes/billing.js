...
const stripeEngine_1 = require("../billing/stripeEngine");
 ...
 */
exports.billingRouter.get('/estimate', (req, res) => {
    const beds = parseInt(req.query.beds, 10) || 60;
    const census = parseInt(req.query.census, 10) || beds;
    const fee = stripeEngine_1.StripeBillingEngine.calculateMonthlyFee(beds, census);
    res.json(fee);
});
/**
 ...
 */
exports.billingRouter.post('/webhook', (req, res, next) => {
    try {
        ...
        try {
            ...
            console.warn(`[billing] Rejected Stripe webhook: ${err.message}`);
            return res.status(400).json({ error: 'Invalid Stripe webhook signature' });
        }
        const event = JSON.parse(rawBody.toString('utf8'));
        const result = stripeEngine_1.StripeBillingEngine.handleStripeWebhookEvent(event);
