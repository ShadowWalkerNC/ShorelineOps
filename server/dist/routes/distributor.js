...
exports.distributorRouter = void 0;
...
exports.distributorRouter.get('/vendors', async (_req, res, next) => {
    try {
...
exports.distributorRouter.get('/catalog', async (req, res, next) => {
    const { vendorId } = req.query;
    if (!vendorId)
        return err(res, 400, 'vendorId is required');
    try {
        const { rows } = await pool_1.pool.query(`
      SELECT vi.*,
             vim.id AS match_id,
...
function parseCsvCatalog(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (parts.length <= skuIdx || !parts[skuIdx])
            continue;
...
exports.distributorRouter.post('/catalog-upload', async (req, res, next) => {
    ...
    try {
        ...
        const { rows: canonicalRows } = await pool_1.pool.query('SELECT * FROM canonical_products');
        ...
        for (const item of itemsToProcess) {
            ...
            if (existing.length > 0) {
                ...
                await pool_1.pool.query(`UPDATE vendor_items SET
             name = $1, brand = $2, pack_size = $3, uom = $4, category = $5, unit_cost = $6, active = true, updated_at = NOW()
           WHERE id = $7`, [item.name, item.brand || '', item.packSize || '', item.uom || 'case', item.category || '', item.unitCost, itemId]);
            }
            else {
                itemId = (0, crypto_1.randomUUID)();
                await pool_1.pool.query(`INSERT INTO vendor_items (id, vendor_id, vendor_sku, name, brand, pack_size, uom, category, unit_cost, active)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, true)`, [itemId, vendorId, item.sku, item.name, item.brand || '', item.packSize || '', item.uom || 'case', item.category || '', item.unitCost]);
            }
            importedCount++;
...
exports.distributorRouter.get('/canonical-products', async (_req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
      SELECT cp.*,
...
exports.distributorRouter.post('/canonical-products', async (req, res, next) => {
    const { name, category, standardUom, description = '', allergens = [] } = req.body;
    if (!name || !category || !standardUom)
        return err(res, 400, 'name, category, and standardUom are required');
    try {
        const id = (0, crypto_1.randomUUID)();
        const { rows } = await pool_1.pool.query(`INSERT INTO canonical_products (id, name, category, standard_uom, description, allergens)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [id, name, category, standardUom, description, allergens]);
...
exports.distributorRouter.post('/match', async (req, res, next) => {
    ...
    try {
        ...
        const { rows: [item] } = await pool_1.pool.query('SELECT * FROM vendor_items WHERE id = $1', [vendorItemId]);
        if (!item)
            return err(res, 404, 'Vendor item not found');
        const norm = catalogMatcher_1.PackSizeNormalizer.normalize(item.pack_size || '', item.unit_cost);
        // Check if match already exists
        const { rows: existing } = await pool_1.pool.query('SELECT id FROM vendor_item_matches WHERE canonical_product_id = $1 AND vendor_item_id = $2', [canonicalProductId, vendorItemId]);
        if (existing.length > 0) {
            await pool_1.pool.query('UPDATE vendor_item_matches SET match_status = $1, pack_quantity_in_standard_uom = $2, normalized_unit_cost = $3, normalized_uom = $4, matched_by = $5, updated_at = NOW() WHERE id = $6', [matchStatus, norm.totalStandardUnits, norm.normalizedUnitCost, norm.standardUom, 'manual_desk', existing[0].id]);
        }
        else {
            await pool_1.pool.query('INSERT INTO vendor_item_matches (id, canonical_product_id, vendor_item_id, pack_quantity_in_standard_uom, normalized_unit_cost, normalized_uom, match_confidence, match_status, matched_by) VALUES ($1, $2, $3, $4, $5, $6, 100.0, $7, $8)', [(0, crypto_1.randomUUID)(), canonicalProductId, vendorItemId, norm.totalStandardUnits, norm.normalizedUnitCost, norm.standardUom, matchStatus, 'manual_desk']);
        }
...
exports.distributorRouter.get('/price-matrix', async (_req, res, next) => {
    try {
        const { rows: canonicalRows } = await pool_1.pool.query('SELECT * FROM canonical_products ORDER BY category ASC, name ASC');
...
exports.distributorRouter.get('/orders', async (req, res, next) => {
