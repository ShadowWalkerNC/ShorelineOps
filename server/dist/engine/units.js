...
exports.UnitConversionEngine = exports.INGREDIENT_DENSITIES = exports.COUNT_TO_GRAMS_DEFAULT = exports.VOLUME_TO_ML = exports.MASS_TO_GRAMS = void 0;
// Mass in grams
exports.MASS_TO_GRAMS = {
    g: 1,
    kg: 1000,
    oz: 28.3495,
    lb: 453.592,
...
exports.COUNT_TO_GRAMS_DEFAULT = {
...
class UnitConversionEngine {
     ...
     */
    static parseQuantityString(raw) {
    ...
    static normalizeUnit(unit) {
        ...
        if (['tbsp', 'tablespoon', 'tablespoons', 'tbs'].includes(u))
            return 'tbsp';
        if (['fl oz', 'floz', 'fluid ounce', 'fluid ounces'].includes(u))
            return 'fl oz';
        ...
        if (['gal', 'gallon', 'gallons'].includes(u))
            return 'gal';
        if (['#10', '#10 can', '#10 cans', '10 can'].includes(u))
            return '#10 can';
        if (['can', 'cans'].includes(u))
            return 'can';
        if (['case', 'cases', 'cs'].includes(u))
            return 'case';
        if (['pack', 'packs', 'pk'].includes(u))
            return 'pack';
        if (['bag', 'bags'].includes(u))
            return 'bag';
    ...
    static convert(amount, fromUnit, toUnit, ingredientName) {
        ...
        // 1. Mass to Mass
        if (from in exports.MASS_TO_GRAMS && to in exports.MASS_TO_GRAMS) {
            const grams = amount * exports.MASS_TO_GRAMS[from];
            const result = grams / exports.MASS_TO_GRAMS[to];
            return { convertedAmount: Math.round(result * 1000) / 1000, unit: to };
        }
        // 2. Volume to Volume
        if (from in exports.VOLUME_TO_ML && to in exports.VOLUME_TO_ML) {
            const ml = amount * exports.VOLUME_TO_ML[from];
            const result = ml / exports.VOLUME_TO_ML[to];
            return { convertedAmount: Math.round(result * 1000) / 1000, unit: to };
        }
        // 3. Density lookup for Volume <-> Mass conversions
        const density = this.getDensity(ingredientName);
        if (from in exports.VOLUME_TO_ML && to in exports.MASS_TO_GRAMS) {
            const ml = amount * exports.VOLUME_TO_ML[from];
            const grams = ml * density;
            const result = grams / exports.MASS_TO_GRAMS[to];
            return { convertedAmount: Math.round(result * 1000) / 1000, unit: to };
        }
        if (from in exports.MASS_TO_GRAMS && to in exports.VOLUME_TO_ML) {
            const grams = amount * exports.MASS_TO_GRAMS[from];
        ...
        if (from === '#10 can' && to in exports.MASS_TO_GRAMS) {
            const grams = amount * (exports.COUNT_TO_GRAMS_DEFAULT['#10 can'] ?? 2948.35);
            return { convertedAmount: grams / exports.MASS_TO_GRAMS[to], unit: to };
        }
        if (from in exports.MASS_TO_GRAMS && to === '#10 can') {
            const grams = amount * exports.MASS_TO_GRAMS[from];
            const cans = grams / (exports.COUNT_TO_GRAMS_DEFAULT['#10 can'] ?? 2948.35);
     ...
     */
    static getDensity(ingredientName) {
        if (!ingredientName)
            return 1.0;
        const lower = ingredientName.toLowerCase();
        for (const [key, density] of Object.entries(exports.INGREDIENT_DENSITIES)) {
...
exports.UnitConversionEngine = UnitConversionEngine;
