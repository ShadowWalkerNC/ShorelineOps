...
exports.AVG_USAGE_WINDOW_DAYS = void 0;
exports.rollupAvgUsage = rollupAvgUsage;
exports.startNightlyForecastRollup = startNightlyForecastRollup;
exports.stopNightlyForecastRollup = stopNightlyForecastRollup;
/**
...
exports.AVG_USAGE_WINDOW_DAYS = 28;
async function rollupAvgUsage() {
    const cutoff = new Date(Date.now() - exports.AVG_USAGE_WINDOW_DAYS * 86_400_000)
        .toISOString()
        .slice(0, 10);
    ...
    const { rows } = await pool_1.pool.query(`SELECT vi.vendor_id AS vendor_id,
     ...
     JOIN inventory_transactions t
      ...
      AND t.created_at >= $1
     GROUP BY vi.vendor_id, vi.id`, [cutoff]);
    let updated = 0;
    for (const r of rows) {
        const avg = parseFloat(r.consumed) / exports.AVG_USAGE_WINDOW_DAYS;
        await pool_1.pool.query(`UPDATE order_guides
       SET avg_usage = $1, updated_at = NOW()
       WHERE vendor_id = $2 AND vendor_item_id = $3`, [avg, r.vendor_id, r.vendor_item_id]);
        updated += 1;
    }
    const result = {
        updated,
        windowDays: exports.AVG_USAGE_WINDOW_DAYS,
        cutoff,
        ranAt: new Date().toISOString(),
    };
    console.log(`[nightly-forecast] avg_usage rollup: updated ${updated} order-guide line(s), ` +
        `window ${exports.AVG_USAGE_WINDOW_DAYS}d (since ${cutoff})`);
    return result;
}
 ...
 */
function startNightlyForecastRollup(intervalMs = 24 * 60 * 60 * 1000) {
    if (rollupTimer)
        return;
...
function stopNightlyForecastRollup() {
    if (rollupTimer) {
