...
exports.EXPECTED_TABLES = void 0;
exports.assertSchemaIntegrity = assertSchemaIntegrity;
...
async function assertSchemaIntegrity() {
    ...
    if (missing.length > 0) {
        throw new Error(`[schema-drift] FATAL: ${missing.length} expected table(s) missing from the database: ${missing.join(', ')}. ` +
            `The database schema is out of sync with server/src/db/migrate.ts. Refusing to boot.`);
    }
    console.log(`[migrate] Schema integrity check passed (${exports.EXPECTED_TABLES.length}/${exports.EXPECTED_TABLES.length} tables present)`);
}
async function runMigrations(maxRetries = 5, retryDelayMs = 2000) {
    let attempt = 0;
    ...
    for (const { name, sql } of migrations) {
        const { rows } = await pool_1.pool.query('SELECT 1 FROM _migrations WHERE name = $1', [name]);
        if (rows.length > 0) {
