...
exports.isDemoSeedEnabled = isDemoSeedEnabled;
exports.assertDemoSeedAllowed = assertDemoSeedAllowed;
...
function generateCatalog(vendorId, vendorPrefix, count = 260) {
    const items = [];
...
function assertDemoSeedAllowed() {
    if (!isDemoSeedEnabled()) {
        throw Object.assign(new Error(process.env.NODE_ENV === 'production'
            ? 'Demo seeding is forbidden in production.'
            : 'Demo seeding requires explicit SHORELINE_DEMO_SEED=true in a non-production environment.'), { status: 403 });
    }
}
async function runSeed() {
    ...
    const hash = await bcryptjs_1.default.hash(defaultPass, 10);
    const USERS_TO_SEED = [
    ...
    for (const u of USERS_TO_SEED) {
        await pool_1.pool.query(`INSERT INTO users (id, name, email, password, role)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (email) DO NOTHING`, [crypto_1.default.randomUUID(), u.name, u.email, hash, u.role]);
    }
    ...
    for (const f of FACILITIES) {
        await pool_1.pool.query(`INSERT INTO facility_config (id, facility_name, primary_contact_email, facility_type, address, wings, dining_rooms, is_initialized)
       VALUES ($1, $2, $3, $4, $5, $6, $7, true)
       ON CONFLICT (id) DO NOTHING`, [f.id, f.name, f.email, f.type, f.address, f.wings, f.dining]);
    }
    ...
    await pool_1.pool.query(`INSERT INTO facility_config (id, facility_name, primary_contact_email, facility_type, address, wings, dining_rooms, is_initialized)
     ...
     ON CONFLICT (id) DO NOTHING`, [FACILITIES[0].wings, FACILITIES[0].dining]);
    ...
    for (const r of residentsList) {
       ...
       ON CONFLICT DO NOTHING`, [
            crypto_1.default.randomUUID(), r.name, r.room, r.status, r.diet_type, r.texture, r.portion_size,
    ...
    for (const v of VENDORS) {
        await pool_1.pool.query(`INSERT INTO vendors (id, name, code, website, notes)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (code) DO NOTHING`, [crypto_1.default.randomUUID(), v.name, v.code, v.website, v.notes]);
        let { rows } = await pool_1.pool.query('SELECT id FROM vendors WHERE code = $1', [v.code]);
        if (rows[0] && !rows[0].id) {
            const generatedId = crypto_1.default.randomUUID();
            await pool_1.pool.query('UPDATE vendors SET id = $1 WHERE code = $2', [generatedId, v.code]);
    ...
    for (const item of allCatalogItems) {
       ...
       ON CONFLICT (vendor_id, vendor_sku) DO NOTHING`, [itemId, item.vendorId, item.sku, item.name, item.brand, item.packSize, item.uom, item.category, item.unitCost]);
        insertedCount++;
    }
    ...
    for (const staple of CANONICAL_STAPLES) {
        const canonId = crypto_1.default.randomUUID();
        await pool_1.pool.query(`INSERT INTO canonical_products (id, name, category, standard_uom, allergens)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT DO NOTHING`, [canonId, staple.name, staple.category, staple.standardUom, staple.allergens]);
        const { rows: [canon] } = await pool_1.pool.query('SELECT id FROM canonical_products WHERE name = $1', [staple.name]);
        if (!canon)
            continue;
        // Find candidate vendor items from Dennis and Sysco
        const { rows: candidateItems } = await pool_1.pool.query(`SELECT vi.*, v.code as vendor_code
       FROM vendor_items vi
       JOIN vendors v ON v.id = vi.vendor_id
       WHERE vi.name LIKE $1
       LIMIT 4`, [`%${staple.keyword}%`]);
        for (const vItem of candidateItems) {
            // Approximate pack quantity based on standardUom
            let packUnits = 10;
            if (staple.standardUom === 'gal')
                packUnits = 4;
            else if (staple.standardUom === 'lb')
                packUnits = vItem.pack_size.includes('20') ? 20 : 10;
            else
                packUnits = 6;
            ...
            await pool_1.pool.query(`INSERT INTO vendor_item_matches
         ...
         ON CONFLICT (canonical_product_id, vendor_item_id) DO NOTHING`, [crypto_1.default.randomUUID(), canon.id, vItem.id, packUnits, normalizedCost]);
            matchCount++;
        }
    }
    ...
    const { rows: recipeCount } = await pool_1.pool.query('SELECT COUNT(*) FROM recipes');
...
if (require.main === module) {
    Promise.resolve().then(() => __importStar(require('dotenv/config'))).then(() => runSeed()
