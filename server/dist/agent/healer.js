...
class OperationsHealerBot {
    ...
    async runAudit(autoFix = true) {
        ...
        try {
            ...
            const { rows: outOfRange } = await pool_1.pool.query(`SELECT id, temp_f, item_name FROM haccp_logs
         WHERE recorded_at >= $1 AND recorded_at < $2 AND compliant = false`, [dayStart.toISOString(), dayEnd.toISOString()]);
