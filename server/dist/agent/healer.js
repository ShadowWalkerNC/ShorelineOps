...
exports.globalHealerBot = exports.OperationsHealerBot = void 0;
const pool_1 = require("../db/pool");
const cache_1 = require("../middleware/cache");
class OperationsHealerBot {
    timer = null;
    /**
     * Run comprehensive self-healing diagnostics across all subsystems
     */
    async runAudit(autoFix = true) {
        const checks = [];
        let remediationsCount = 0;
        // 1. Database & Pool Connection Health
        try {
            const start = Date.now();
            const { rows } = await pool_1.pool.query('SELECT 1 as ping');
        ...
        let activeCensus = 0;
        try {
            const { rows: residents } = await pool_1.pool.query('SELECT id, name, diet_type, texture FROM residents');
        ...
        try {
            const cacheSize = cache_1.serverCache.size;
            checks.push({
        ...
        try {
            ...
            const { rows: tables } = await pool_1.pool.query("SELECT name FROM sqlite_master WHERE type='table' AND name='food_temperatures'");
            if (tables.length > 0) {
                const { rows: tempLogs } = await pool_1.pool.query("SELECT id, temperature, item_name FROM food_temperatures WHERE DATE(logged_at) = $1", [today]);
...
exports.OperationsHealerBot = OperationsHealerBot;
exports.globalHealerBot = new OperationsHealerBot();
