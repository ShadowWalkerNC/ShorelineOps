...
function TallyBadges({ t }: { t: DietTally }) {
  const items = [
    ['Diabetic Modifications', t.diabetic],
    ['Cut-Up', t.cutUp],
    ['Minced', t.minced],
    ['Puréed', t.pureed],
...
function WorksheetTab() {
  ...
  residents.forEach(r => {
    if (r.dietModifications?.includes('Diabetic')) tally.diabetic++
    if (r.dietModifications?.includes('Cut-Up'))   tally.cutUp++
    if (r.dietModifications?.includes('Minced'))   tally.minced++
    if (r.dietModifications?.includes('Pureed'))   tally.pureed++
    ...
    if (r.supplement === 'Ensure')                  tally.ensure++
  })

  const total = residents.filter(r => r.status === 'active').length
  const diningRoom = residents.filter(r => r.servingLocation === 'Dining Room').length
  const roomService = residents.filter(r => r.servingLocation === 'Room').length
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
            {(['Option A', 'Option B'] as const).map((opt, oi) => (
              <div key={opt} style={{ background: 'var(--bg-app)', borderRadius: 'var(--radius-md)', padding: '10px 14px', border: '1px solid var(--border-color)' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 2 }}>{opt} <span style={{ fontWeight: 400 }}>(Planned Tally: {oi === 0 ? total : 0} portions)</span></div>
          ...
          <div key={r.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Room {r.room ?? '—'} · Table {r.tableNumber ?? '—'}</div>
              </div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 3 }}>
              <span>Diet: <b>{r.dietType ?? 'Regular'}</b></span>
              {r.allergies?.length ? <span style={{ color: '#dc2626' }}>⚠ Allergies: {r.allergies.join(', ')}</span> : null}
              {r.beverages ? <span>Beverages: {r.beverages}</span> : null}
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', padding: 16, display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
        <div style={{ flex: '2 1 200px' }}>
          ...
          <input style={inputStyle} value={newTask} onChange={e => setNewTask(e.target.value)} placeholder="e.g. Prep salad bar" />
        </div>
        <div style={{ flex: '1 1 140px' }}>
          ...
          <input style={inputStyle} value={newAssignee} onChange={e => setNewAssignee(e.target.value)} placeholder="Staff name" />
        </div>
        <div style={{ flex: '1 1 120px' }}>
          ...
          <select style={inputStyle} value={newMeal} onChange={e => setNewMeal(e.target.value as MealSlot)}>
...
function EnsureTab() {
  const { residents } = useResidentsStore()
  const ensureResidents = residents.filter(r => r.supplement === 'Ensure' && r.status === 'active')
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {ensureResidents.map(r => (
          <div key={r.id} onClick={() => toggle(r.id)} style={{
            background: checked.has(r.id) ? 'var(--color-success-light,#f0fdf4)' : 'var(--bg-card)',
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Room {r.room ?? '—'} · {r.ensureCans ?? 1} can{(r.ensureCans ?? 1) > 1 ? 's' : ''}</div>
            </div>
        ...
        {tasks.map(task => {
          ...
          return (
            <div key={key} onClick={() => toggle(key)} style={{
              background: isDone ? 'var(--color-success-light,#f0fdf4)' : 'var(--bg-card)',
...
export default function ProductionPage() {
  ...
  useEffect(() => { fetchResidents(); fetchWeeks() }, [])
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      {/* Sub-nav buttons matching screenshots */}
