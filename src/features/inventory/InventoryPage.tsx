...
// INVENTORY PAGE
// ============================================================
// All state now lives in useInventoryStore (Zustand).
// Edits persist across navigation and are visible to
...
const WASTE_REASONS: WasteReason[] = ['Overproduction', 'Plate Waste', 'Expired', 'Contamination', 'Other']
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
      ...
      {showAdd && (
        <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-4)', display: 'flex', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
          ...
          <div style={{ flex: '1 1 140px' }}><label>Category</label><select className="sl-select" value={newItem.category} onChange={e => setNewItem(p => ({ ...p, category: e.target.value as InventoryCategory }))}>{INVENTORY_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'flex-end', flexShrink: 0 }}>
            <button onClick={handleAdd} className="btn btn-primary">Save</button>
...
function WasteTab() {
  ...
  const totalCost = wasteEntries.reduce((s, e) => s + (e.cost ?? 0), 0)

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
      <div style={{ display: 'flex', gap: 'var(--space-4)', flexWrap: 'wrap' }}>
        <div className="sl-stat-card">
          <div className="sl-eyebrow">Waste Entries</div>
          <div style={{ fontSize: 'var(--text-3xl)', fontWeight: 'var(--weight-black)', fontFamily: 'var(--font-display)', color: 'var(--color-primary)' }}>{wasteEntries.length}</div>
        </div>
        <div className="sl-stat-card">
          <div className="sl-eyebrow">Estimated Waste Cost</div>
          <div style={{ fontSize: 'var(--text-3xl)', fontWeight: 'var(--weight-black)', fontFamily: 'var(--font-display)', color: '#dc2626' }}>{fmt$(totalCost)}</div>
        </div>
      </div>
      ...
      {showForm && (
        <div style={{ background: 'var(--bg-app)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-4)', display: 'flex', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
          ...
          <div style={{ flex: '1 1 130px' }}><label>Reason</label><select className="sl-select" value={form.reason} onChange={e => setForm(p => ({ ...p, reason: e.target.value as WasteReason }))}>{WASTE_REASONS.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        {wasteEntries.map(e => {
          const rc: Record<WasteReason, string> = {
            Overproduction: '#d97706', 'Plate Waste': '#7c3aed',
            Expired: '#dc2626', Contamination: '#9f1239', Other: '#64748b',
          }
          return (
...
function ZeroBalanceTab() {
  const { user } = useAuth()
  const { profiles } = useStaffStore()
  const { stockItems, counts, addCount, updateCount } = useInventoryStore()
  ...
  function startNewCount() {
    const items: CountItem[] = stockItems
      ...
      .map(i => ({ id: uid(), itemId: i.id, itemName: i.item, unit: i.unit, expected: i.qty, counted: '' as const, variance: 0, note: '' }))
    setActiveCount({ id: uid(), countDate: TODAY, submittedById: myStaffId, status: 'Draft', items, notes: '' })
  }
              ...
              <div key={c.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {isPrivileged && c.status === 'Submitted' && (
                    <button onClick={() => updateCount(c.id, { status: 'Approved' })} className="btn btn-primary btn-sm">Approve</button>
                  )}
                  {isPrivileged && c.status === 'Discrepancy' && (
                    <button onClick={() => updateCount(c.id, { status: 'Approved' })} className="btn btn-outline btn-sm" style={{ borderColor: '#dc2626', color: '#dc2626' }}>Resolve &amp; Approve</button>
                  )}
                </div>
              </div>
            ))}
                ...
                {activeCount.items.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id} style={{ background: hasVar ? '#fef2f2' : (i % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-app)'), borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={TD}>
                        <input type="number" min={0} value={item.counted === '' ? '' : item.counted} onChange={e => updateCounted(item.id, e.target.value)} className="sl-input" style={{ width: 80 }} placeholder="count" />
                      </td>
