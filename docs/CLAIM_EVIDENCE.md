...
| Access controls and approval workflows | EHR reconciliation is dietitian/admin-gated and atomic; purchasing lines are draft-bound with manager approval. Covered by `ehr-decision.test.ts` and `purchasing-integrity.test.ts` using disposable SQLite databases. PostgreSQL concurrency remains acceptance work. |
